function MM1FCFSCap(n,mi,lambda,x0,x1,cap)
%Simulación de un sistema MM1. FCFS y un servidor y capacidad finita

%     n: Número de eventos
%    mi: Media del tiempo de servicios 
%lambda: Tasa de llegadas
%x0--x1: Estado del sistema durante los servicios x1-x0
%   cap: Capacidad del servidor
...
switch tipoG
    case 1
        if Ti(k)>=Ts(i)+Ti(i)
            ...
            Nt(k+i,:)=[((Nt(k+i-1,1)+1)<=cap).*1+(Nt(k+i-1,1)),Ti(k+1)];
            k=k+1;
        end
    case 2
        Ts(i+1)=-log(rand())/mi;
        ...
        Nt(k+i,:)=[Nt(k+i-1,1)-(Nt(k+i-1,1)>0),Te(i+1)+Ti(i+1)];
        i=i+1;
        tipoG=1;
end
end
